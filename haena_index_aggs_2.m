...
shg
% Import video 
cd 'C:\Users\Andrew''s Laptop\Downloads\'

v = VideoReader('barely_move_50.avi');
frames = 50; %v.NumFrame;
%frames = 10;
centx = []; centy = []; area = [];


video = VideoWriter('barely_move_50_edit_remove2.avi');
video.FrameRate = 1;
...
for i = 1:50
    ...
    h1 = imbinarize(h1, 0.15); %binarize, remove tickmarks in image
    
    [L1,N] =bwlabel(h1); %labels each aggregate with a unique number in the matrix
                        %N is the total number of objects
    num = [num; N]
    temp_a1 = regionprops(L1,'Area');
...
%     user_area = 100;
%     %lit_aggs = []; sad_aggs = []; %check
%     for k = 1:N
%         if struct2array(temp_a1(k)) <= user_area
%             %sad_aggs = [sad_aggs; k]; %store bad agg - check
%             [r,c] = find(L1 == k); %pull out that aggregate and relabel;
%             for z = 1:length(r)
%                 h1(r(z),c(z)) = 0; %make the aggregate just background
%             end            
%         else
%             %store all the indices that meet the criteria - check
%             %lit_aggs = [lit_aggs; k];
%         end
%     end
...
for i = 2:n
    for agg = 1:totalagg
        xprev = centx_new(agg, (i-1)); % 30th aggregate, frame 1
        yprev = centy_new(agg, (i-1)); % 
        for j = 1:size(centx_new, 1)
            xnext = centx(j, i); % 30-31th aggregate, frame 2 
            ynext = centy(j, i); % 30-31th aggregate, frame 2 
